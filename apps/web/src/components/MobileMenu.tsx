...
// A single fixed, exactly-viewport-sized container (inset: 0) that holds the
// overlay and the slide-in panel. Because it's the drawer's *own* clipping
// context (`overflow: hidden`) and never larger than the viewport, the panel —
// which is absolutely positioned *inside* it and slides in from off its right
// edge — cannot widen the document or produce a horizontal scrollbar. When
// closed the whole thing is inert (no pointer events) and, once the slide-out
// finishes, `visibility: hidden` so its links leave the keyboard tab order.
